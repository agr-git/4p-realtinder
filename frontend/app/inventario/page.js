...
function toRecord(r) {
  const tipo = (r.tipo || "").trim().toLowerCase() || null;
  const precio = int(r.precio_cop);
  if (!tipo && precio == null) return null;
  const id = `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 7)}`;
  return {
    dedupe_key: `propio:${id}`, source: "propio", source_listing_id: id, url: r.url || null,
    business_type: (r.negocio || "venta").toLowerCase(), property_type: tipo, title: r.titulo || null,
    price_cop: precio, beds: int(r.habitaciones), baths: int(r.banos), area_m2: num(r.area_m2),
    city: (r.ciudad || "").toLowerCase() || null, neighborhood: (r.barrio || "").toLowerCase() || null,
    features: r.parqueadero ? { parqueadero: r.parqueadero } : null, scraped_at: new Date().toISOString(),
    // owner lo pone auth.uid() por default+RLS.
  };
}
...
export default function Inventario() {
  ...
  const insert = async (records, label) => {
  ...
  const downloadTemplate = () => {
    ...
    const a = document.createElement("a"); a.href = "data:text/csv;charset=utf-8," + encodeURIComponent(csv); a.download = "plantilla_inventario_realtinder.csv"; a.click();
  ...
  const addOne = async () => {
    ...
    await insert([form]); setForm(empty); setModal(false);
  };

  if (session === null) return <div className="wrap"><div className="empty" style={{ marginTop: "var(--sp-5)" }}>Inicia sesión (arriba a la derecha) para subir y ver tu inventario.</div></div>;
  ...
  return (
    <div className="wrap">
      ...
      <div className="inv-note">Estos inmuebles se guardan en tu cuenta y aparecen en tu búsqueda junto a los scrapeados.</div>
          ...
          {rows == null ? <tr><td colSpan={9} style={{ textAlign: "center", padding: 30, color: "var(--muted)" }}>Cargando…</td></tr>
            ...
            : rows.map((r) => (
              <tr key={r.id}><td>{cap(r.property_type)}</td><td>{r.business_type}</td><td className="tnum">{fmt(r.price_cop)}</td>
                ...
                <td>{cap(r.city)}</td><td>{cap(r.neighborhood)}</td><td>{r.features?.parqueadero || "—"}</td></tr>
            ))}
        </tbody>
      </table></div>
