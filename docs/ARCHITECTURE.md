...
| **Independent SSR (4)** | fincaraiz (Next.js SSR), ciencuadras (Scully), puntopropiedad (LifullConnect), pads (Astro), casasyapartamentosmanizales (custom CMS) | One selector set each. Same n8n loop, different config entries. |
...
│   ├── TECH_STACK.md
│   ├── ARCHITECTURE.md
│   └── PROJECT_BLUEPRINT.md        (generated by blueprint skill)
├── db/
│   ├── migrations/                 (numbered .sql files, applied via Supabase)
│   └── seed/
├── n8n-workflows/
│   ├── scrape-ssr-sites.json       (exported workflow — versioned, reviewable)
│   └── site-configs/               (selector maps as JSON, one per site group)
├── frontend/                       (Next.js app — Vercel deploys from here)
│   ├── app/
│   ├── components/
│   └── lib/
└── README.md
...
Branching: `main` (auto-deploys to Vercel) ← PR ← `feat/*`, `fix/*` branches.
Issues labeled by phase (`phase-1` … `phase-5`).
